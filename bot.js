...
async function loadLeagues() {
  const season = seasonYear(), previous = season - 1, all = [];
  for (const league of LEAGUES) {
    const previousRows = await loadFeed(`${league.slug}-${previous}`);
    const currentRows = await loadFeed(`${league.slug}-${season}`);
    const previousMatches = previousRows.map(r => normalize(r, league.name)).filter(m => m.home && m.away);
    const currentMatches = currentRows.map(r => normalize(r, league.name)).filter(m => m.home && m.away);
    const previousPlayed = previousMatches.filter(m => m.played);
    const currentPlayed = currentMatches.filter(m => m.played);
    const targets = selectNextMatchday(currentMatches.filter(m => !m.played && m.parsedDate));
    if (!targets.length) { console.log(`${league.name}: nessuna partita futura trovata.`); continue; }
    const surv = survivalProfile(previousPlayed), la = leagueAvg(currentPlayed, previousPlayed);
    for (const match of targets) {
      const homeProfile = profile(match.home, "home", currentPlayed, previousPlayed, surv, la);
...
async function run() {
  ...
  } catch (err) { console.error("Errore esecuzione bot:", err); process.exitCode = 1; }
}
